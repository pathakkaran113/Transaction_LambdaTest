...
//     const query = 'SELECT SUM(amount) AS total_amount FROM bankDetails2 WHERE debit_or_credit = "debited"';
...
//   const queryTotal = 'SELECT SUM(amount) AS total_amount FROM bankDetails2 WHERE debit_or_credit = "debited"';
...
//       // Log the total amount to ensure it's fetched correctly
//       console.log('Total Amount:', totalResults);

//       db.query(queryTransactions, [`%${search}%`], (err, transactionResults) => {
//           if (err) {
//               console.error('Error fetching transactions:', err);
//               return res.status(500).send('An error occurred while fetching transactions.');
//           }

//           // Log the transactions to ensure they are fetched correctly
//           console.log('Transactions:', transactionResults);

//           res.render('transactionskp.ejs', { 
//               transactions: transactionResults, 
//               //total: totalResults[0]?.total_amount || 0, // Ensure total is defined, default to 0 if undefined
//               queryTotal,
//               search: search, 
...
//       res.render('transactionskp.ejs', { 
//         transactions: transactionResults, 
//         total: 500, // Access total_amount from totalResults
//         search: search, 
//         searchType: searchType 
//       });
//     });
...
//           }
//           if (results.length > 0) {
//               resolve(results[0].transaction_date);  // Resolve with the transaction date if available
//           } else {
...
// app.get('/sum-amounts', (req, res) => {
//   let mp = 6;
//     const query = 'SELECT SUM(amount) AS total_amount FROM bankDetails2 WHERE debit_or_credit = "debited" AND MONTH(transaction_date) = ${mp} AND YEAR(transaction_date) = 2024';
//     db.query(query, (err, results) => {
//         if (err) {
//             console.error('Error fetching total amount:', err);
//             return res.status(500).send('An error occurred while fetching the total amount.');
...
// });
// app.get('/sum-amounts', (req, res) => {
//   const month = req.body(); // You can also get this from req.query or any other source if needed
//   const year = 2024;
